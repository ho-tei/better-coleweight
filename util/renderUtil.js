 ...
 */
export function drawEspBox(x, y, z, red, green, blue, alpha, phase = true) {
  // thanks to renderlib, don't need the whole module so I'm not adding it.
 ...
 */
export function drawCoolWaypoint(
  x,
  y,
  z,
  r,
  ...
  b,
  {
    ...
    drawBox = true,
    nameColor = "a",
    includeVerticalDistance = true,
  }
) {
  if (drawBox) drawEspBox(x + 0.5, y, z + 0.5, r, g, b, alpha, phase);

  if (name || showDist) {
    let distToPlayerSq =
      (x - Player.getRenderX()) ** 2 +
      (y - (Player.getRenderY() + Player.getPlayer()["func_70047_e"]())) ** 2 +
      (z - Player.getRenderZ()) ** 2;
    let distanceText = includeVerticalDistance
      ? Math.hypot(
          x - Player.getRenderX(),
          y - (Player.getRenderY() + Player.getPlayer()["func_70047_e"]()),
          z - Player.getRenderZ()
        )
    ...
    let loc5 = [
      Player.getRenderX() +
        (x + 0.5 - Player.getRenderX()) / (distToPlayer / distRender),
      Player.getRenderY() +
        Player.getPlayer()["func_70047_e"]() +
        (y +
          2 +
          (20 * distToPlayer) / 300 -
          (Player.getRenderY() + Player.getPlayer()["func_70047_e"]())) /
          (distToPlayer / distRender),
      Player.getRenderZ() +
        (z + 0.5 - Player.getRenderZ()) / (distToPlayer / distRender),
    ];
    let loc6 = [
      Player.getRenderX() +
        (x + 0.5 - Player.getRenderX()) / (distToPlayer / distRender),
    ...
    if (showDist)
      Tessellator.drawString(
        "\xA7b(" + addCommas(Math.round(distanceText)) + "m)",
        name ? loc6[0] : loc5[0],
        name ? loc6[1] : loc5[1],
        name ? loc6[2] : loc5[2],
        0,
