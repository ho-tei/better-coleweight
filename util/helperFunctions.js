...
export function waypointRender(waypoints, yellow = false, numbered = false) {
  ...
  waypoints.forEach((waypoint, index) => {
    string =
      Math.floor(
        (Math.abs(parseInt(Player.getX()) - waypoint[0]) +
          Math.abs(parseInt(Player.getY()) - waypoint[1]) +
          Math.abs(parseInt(Player.getZ()) - waypoint[2])) /
          3
      ) + "m";
    if (numbered) string = index + 1;
    ...
    else Tessellator.drawString(string, waypoint[0], waypoint[1], waypoint[2]);
  });
}
...
export function parseNotatedInput(input) {
  ...
  if (parseFloat(input) == input) return parseFloat(input);
...
export function getSelectedProfile(res) {
  for (let i = 0; i < res.profiles.length; i += 1) {
    if (res.profiles[i].selected == true) return res.profiles[i];
  }
}
    ...
    register("renderOverlay", () => {
      ...
      if (this.drawState == 1) {
        ...
        if (this.drawTimestamp == undefined) {
          ...
          title.setAlign("CENTER").setShadow(true).setScale(this.scale).draw();
          this.drawState = 1;
        }
      }
    });
...
class LocationChecker {
  ...
  check() {
    if (Date.now() - this.checkTime > 1000) {
      // 1 sec
      this.checkTime = Date.now();
      ...
      for (let lineIndex = 0; lineIndex < this.scoreboard.length; lineIndex++) {
        for (
          ...
          locationsIndex < this.locations.length;
          locationsIndex++
        ) {
          if (
            this.scoreboard[lineIndex]
              .toString()
              .includes(this.locations[locationsIndex])
          ) {
            this.state = true;
            return this.state;
          }
...
export const hollowsChecker = new LocationChecker([
  "Goblin",
  "Jungle",
  "Mithril",
  "Precursor",
...
export const dwarvenChecker = new LocationChecker([
  "Dwarven",
  "Royal",
  "Palace",
  "Library",
...
export const foragingChecker = new LocationChecker([
  "§aDark Thic",
  "§aBirch Par",
  "§aSpruce Wo",
  "§aSavanna W",
...
export function secondsToMessage(seconds) {
  ...
  else return `${hour}h ${Math.floor(seconds / 60) - hour * 60}m`;
}
...
export function distanceCalc(waypoint1, waypoint2, includeVertical = true) {
  ...
  else return Math.hypot(waypoint1.x - waypoint2.x, waypoint1.z - waypoint2.z);
}
...
export function isPlayerHoldingDrill() {
  ...
  return (
    ...
    (itemId.includes("drill") ||
      itemId.includes("gemstone_gauntlet") ||
      itemId.includes("pickonimbus"))
  );
}
