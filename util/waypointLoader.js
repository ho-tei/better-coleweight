  ...
  try {
    if (data.startsWith("<Skytils-Waypoint-Data>(V"))
      return {
        success: false,
        message:
          "&eLoader doesn't support Skytils V1 (goto ninjune.dev/waypoint &e)",
      };
    ...
    } else if (data.startsWith("{") || data.startsWith("AQAA")) {
      // old soopy I think
      if (data.startsWith("AQAA"))
        tempData = weirdSoopyFormatToRealSoopyFormat(data);
      else tempData = JSON.parse(data);
...
const B64 = {
  ...
  encode: function (input) {
    ...
    while (i < input.length) {
      ...
      output =
        output +
        B64._keyStr.charAt(enc1) +
        B64._keyStr.charAt(enc2) +
        B64._keyStr.charAt(enc3) +
        B64._keyStr.charAt(enc4);
    }
