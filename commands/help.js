  ...
  execute: (args) => {
    ChatLib.chat(
      ChatLib.getCenteredText(
        "&b--------------[ &a&lBetterColeweight &b]--------------"
      )
    );
    ChatLib.chat(
      ChatLib.getCenteredText("&7(Hover over command to see usage.)")
    );
...
export function helpCommand(command, desc, usage) {
  ChatLib.chat(
    new TextComponent(`&a◆ /bcw ${command} => &b${desc}`).setHoverValue(
      `${"/bcw " + command + " " + usage}`
    )
  );
}
