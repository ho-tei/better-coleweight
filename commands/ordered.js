...
const clipTitle = new Title({
  text: "&cClip next",
  scale: 2,
  sound: "mob.bat.hurt",
  yOffset: 50,
});
const etherTitle = new Title({
  text: "&5Etherwarp next",
  scale: 2,
...
export default registerCommand({
  ...
  execute: (args) => {
    if (args[1] == undefined)
      return ChatLib.chat(
        `${constants.PREFIX}&eUnknown usage! Hit tab on "/bcw ordered " to see usages.` // TODO: Usage constant message
      );
    switch (args[1].toLowerCase()) {
      ...
      case "skip":
        ...
        else currentOrderedWaypointIndex++;

        while (
          orderedWaypoints.length > 1 &&
      ...
      case "skipto":
        if (
          parseInt(args[2]) > 0 &&
          parseInt(args[2]) < orderedWaypoints.length
        ) {
          ...
          ChatLib.chat(
            `${constants.PREFIX}&eSkipto must be in range 1 - ${
              orderedWaypoints.length - 1
            }`
          );
        break;
      case "unskip":
        ...
        ChatLib.chat(
          `${constants.PREFIX}&bWent back ${args[2] ?? 1} waypoints.`
        );
        break;
      case "enable":
        enabled = true;
      ...
      case "remove":
        if (orderedWaypoints.length < 1)
          return ChatLib.chat(
            `${constants.PREFIX}&eWaypoints have not been loaded!`
          );
        if (args[2] == undefined)
          return ChatLib.chat(
            `${constants.PREFIX}&bUsage: '/bcw ordered delete (number)'`
          );
        wNum = parseInt(args[2]);
        if (wNum == undefined || wNum < 1 || wNum > orderedWaypoints.length)
          return ChatLib.chat(
            `${constants.PREFIX}&eInvalid number! Must be in range (1 - ${orderedWaypoints.length})`
          );
        for (let i = wNum - 1; i < orderedWaypoints.length; i++)
          orderedWaypoints[i].options.name = (
            parseInt(orderedWaypoints[i].options.name) - 1
          ).toString();
      ...
      case "insert":
        if (args[2] == undefined)
          return ChatLib.chat(
            `${constants.PREFIX}&bStand where you want to add a waypoint (will be block under you) and do '/bcw waypoint insert (number)'`
          );
        ...
        if (wNum == orderedWaypoints.length + 1) {
          ...
          return ChatLib.chat(
            `${constants.PREFIX}&bAdded waypoint ${wNum} at ${wX}, ${wY}, ${wZ}!`
          );
        }
        if (wNum == undefined || wNum < 1 || wNum > orderedWaypoints.length)
          return ChatLib.chat(
        ...
        orderedWaypoints.splice(wNum - 1, 0, {
          x: wX,
          y: wY,
          z: wZ,
          r: 0,
          g: 1,
          b: 0,
          options: { name: wNum.toString() },
        });
        ChatLib.chat(
          `${constants.PREFIX}&bInserted waypoint ${wNum} at ${wX}, ${wY}, ${wZ}!`
        );
        break;
      case "etherwarp":
        if (args[2] == undefined)
          return ChatLib.chat(
            `${constants.PREFIX}&bMarks a vein as etherwarp. Usage: '/bcw ordered etherwarp (number)'`
          );
        wNum = parseInt(args[2]) - 1;
        if (orderedWaypoints[wNum] == undefined)
          return ChatLib.chat(`${constants.PREFIX}&bVein does not exist.`);

        orderedWaypoints[wNum].options.ether = !(
          orderedWaypoints[wNum]?.options?.ether ?? false
        );
        ChatLib.chat(
          `${constants.PREFIX}&bWaypoint ${wNum + 1} is now ${
            orderedWaypoints[wNum].options.ether ? "enabled" : "disabled"
          } to etherwarp.`
        );
      ...
      case "paneclip":
        if (args[2] == undefined)
          return ChatLib.chat(
            `${constants.PREFIX}&bMarks a vein as paneclip. Usage: '/bcw ordered paneclip (number)'`
          );
        wNum = parseInt(args[2]) - 1;
        if (orderedWaypoints[wNum] == undefined)
          return ChatLib.chat(`${constants.PREFIX}&bVein does not exist.`);

        orderedWaypoints[wNum].options.clip = !(
          orderedWaypoints[wNum]?.options?.clip ?? false
        );
        ChatLib.chat(
          `${constants.PREFIX}&bWaypoint ${wNum + 1} is now ${
            orderedWaypoints[wNum].options.ether ? "enabled" : "disabled"
      ...
      case "save":
        if (args[2] == undefined)
          return ChatLib.chat(
            `${constants.PREFIX}&bUsage: /bcw ordered save (name) [description, can be spaced]`
          );
        ...
        routes[args[2]] = {
          ...
          format: "soopy",
          data: JSON.stringify(orderedWaypoints),
        };
        FileLib.write(
          "BetterColeweight",
          "config/routes.json",
      ...
      default:
        return ChatLib.chat(
          `${constants.PREFIX}&eUnknown usage! Hit tab on "/bcw ordered " to see usages.`
        );
    }
  },
});
...
register("renderWorld", () => {
  ...
  for (let i = 0; i < renderWaypoints.length; i++) {
    ...
    b = 0;
    alpha = 0.6;
    if (i == 0) wpColor = settings.orderedPrevColor;
    else if (i == 1) wpColor = settings.orderedCurColor;
    else if (i == 2) wpColor = settings.orderedNextColor;
    else if (i >= 3) {
      r = 1;
      alpha = 0.4;
    }
    ...
    if (orderedWaypoints[renderWaypoints[i]] == undefined) {
      if (settings.debug) console.log(renderWaypoints[i] + " " + i);
      continue;
    }
    drawCoolWaypoint(
      orderedWaypoints[renderWaypoints[i]].x,
      orderedWaypoints[renderWaypoints[i]].y,
      orderedWaypoints[renderWaypoints[i]].z,
      r,
      g,
      b,
      {
        name:
          i < 3 && (settings.orderedSetup || settings.orderedShowText)
  ...
  if (
    !settings.orderedSetup &&
    settings.orderedWaypointsLine &&
    traceWP != undefined
  )
    trace(
      parseInt(traceWP.x) + 0.5,
      parseInt(traceWP.y) + 0.25,
      parseInt(traceWP.z) + 0.5,
      lineColor.getRed() / 255,
      lineColor.getGreen() / 255,
      lineColor.getBlue() / 255,
      lineColor.getAlpha() / 255,
      settings.orderedLineThickness
    );

  const currentWP = orderedWaypoints[renderWaypoints[1]];
  if (settings.orderedSetup && currentWP != undefined && traceWP != undefined) {
    drawLine(
      parseInt(currentWP.x) + 0.5,
      parseInt(currentWP.y) + 2.65,
      parseInt(currentWP.z) + 0.5,
      parseInt(traceWP.x) + 0.5,
      parseInt(traceWP.y) + 0.5,
      parseInt(traceWP.z) + 0.5,
      lineColor.getRed() / 255,
      lineColor.getGreen() / 255,
      lineColor.getBlue() / 255,
      0.5,
      settings.orderedSetupThickness
    );
  }
...
function decideWaypoints() {
  ...
  if (beforeWaypoint != undefined)
    ...
    renderWaypoints.push(
      orderedWaypoints[orderedWaypoints.length - 1].options.name - 1
    );

  let currentWaypoint = orderedWaypoints[currentOrderedWaypointIndex];
  let distanceTo1 = Infinity;
  if (currentWaypoint != undefined) {
    distanceTo1 = Math.hypot(
      Player.getX() - currentWaypoint.x,
      Player.getY() - currentWaypoint.y,
      Player.getZ() - currentWaypoint.z
    );
  ...
  if (nextWaypoint == undefined) {
    if (orderedWaypoints[0] != undefined) nextWaypoint = orderedWaypoints[0];
  ...
  if (nextWaypoint != undefined) {
    distanceTo2 = Math.hypot(
      Player.getX() - nextWaypoint.x,
      Player.getY() - nextWaypoint.y,
      Player.getZ() - nextWaypoint.z
    );
    renderWaypoints.push(nextWaypoint.options.name - 1);
    if (nextWaypoint.options.clip === true) clipTitle.draw();
    else if (nextWaypoint.options.ether === true) etherTitle.draw();
  }

  if (
    lastCloser === currentOrderedWaypointIndex &&
    distanceTo1 > distanceTo2 &&
    distanceTo2 < settings.nextWaypointRange
  ) {
  ...
  orderedWaypoints.forEach((waypoint) => {
    if (
      ...
      Math.hypot(
        Player.getX() - waypoint.x,
        Player.getY() - waypoint.y,
        Player.getZ() - waypoint.z
      ) < 16
    )
...
export function load(route = "") {
  ...
  if (route == "")
    res = getWaypoints(
      Java.type("net.minecraft.client.gui.GuiScreen").func_146277_j(),
      "soopy"
    );
  else res = getWaypoints(route, "soopy");
  ...
  if (res.success) {
    ...
    ChatLib.chat(
      `${constants.PREFIX}&eThere was an error parsing waypoints! ${res.message}`
    );

  for (let i = 0; i < orderedWaypoints.length - 1; i++) {
    for (let j = 0; j < orderedWaypoints.length - i - 1; j++) {
      if (
        parseInt(orderedWaypoints[j].options.name) >
  ...
  for (let i = 1; i < orderedWaypoints.length; i++)
    if (parseInt(orderedWaypoints[i]?.options?.name) != i + 1)
      ChatLib.chat(
        `${constants.PREFIX}&bNote: Waypoint ${
          i + 1
        } is not in the right order or is not a number! Current is: ${parseInt(
          orderedWaypoints[i]?.options?.name
        )}`
      );
}
