...
let statsHoverCheck = false,
  ...
  pristine,
  blueCheese = false,
  blockPercentage = 0.45;

registerCommand({
  aliases: ["gemstone", "gem"],
  description:
    "$/hr with gemstones. &cLess accurate than Skyhelper's. &4&lDeprecated",
  options: "",
  category: "info",
  execute: (args) => {
    let textComponents,
    ...
    switch (args[1]) {
      case undefined:
        ...
        new TextComponent(
          `${PREFIX}&bHold whatever drill you plan to use, and hover over your profile in /sbmenu. &7[&eNEXT&7]`
        )
    ...
    if (run) {
      const result = findGemstonesPerHr(
        block,
        pristine,
        fortune,
        speed,
      ...
      instaSellBZPrice(`FINE_${block.toUpperCase()}_GEM`).then((bzPrice) => {
        ChatLib.chat(
          `${PREFIX}&b$${addCommas(
            Math.round((result.gemstonesPerHour * bzPrice) / 80 / 80)
          )}/hr (&b~${addCommas(
            Math.round(result.gemstonesPerHour)
          )} ${capitalizeFirst(block)} per hour.)`
        );
      });
    }
  },
...
/**
 * ! Look into current accuracy of calculations -> outdated for over a year!
 */
function findGemstonesPerHr(
  block,
  pristine,
  fortune,
  speed,
  ...
  const miningTicks =
    miningTicksBlocks * percentage + miningTicksShards * (1 - percentage);
  ...
  const msbTicks =
    msbTicksBlocks * percentage + msbTicksShards * (1 - percentage);

  const blocksPerHour =
    (72000 / (1 + miningTicks)) * ((120 - msbTime) / 120) +
    (72000 / (1 + msbTicks)) * (msbTime / 120);

  const gemstonesPerHour =
    avgDrops * (1 + pristine * 0.79) * (1 + fortune / 100) * blocksPerHour;
  return {
    ticks: {
      blocks: miningTicksBlocks,
      shards: miningTicksShards,
      msbBlocks: msbTicksBlocks,
      msbShards: msbTicksShards,
    },
    gemstonesPerHour,
  };
}
