...
registerCommand({
  aliases: ["structure"],
  description: [
    "Generates a structure check route using the route on the clipboard.",
  ],
  options: "(amount)",
  category: "waypoints",
  execute: (args) => {
    ...
    for (let i = 0; i < points; i++) {
      ...
      for (let j = 0; j < Math.floor(waypointAmount / points); j++) {
        if (i * Math.floor(waypointAmount / points) + j >= waypointAmount)
          break;
    ...
    ChatLib.chat(
      `${constants.PREFIX}&bCopied structure check route to clipboard!`
    );
  },
});
