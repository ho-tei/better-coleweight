...
export function hotmCalc(hotmName, minLevel, maxLevel) {
  if (hotmName == undefined) {
    let hotmData = JSON.parse(
      FileLib.read("BetterColeweight", "data/hotm.json")
    ).data;

    ChatLib.chat(
      "/bcw calc hotm (hotmName listed below) (minLevel) [maxLevel]"
    );
  ...
  new Thread(() => {
    ...
    if (minLevel != parseInt(minLevel) || maxLevel != parseInt(maxLevel))
      return ChatLib.chat(constants.CALCULATEERRORMESSAGE);

    minLevel = parseInt(minLevel);
    maxLevel = parseInt(maxLevel);
    let hotmObjectToFind = findHotmObject(hotmName);
    if (hotmObjectToFind == undefined)
      return ChatLib.chat(
        `${PREFIX}&cDid not find HOTM perk with name '${hotmName}'!`
      );

    maxLevel =
      maxLevel < hotmObjectToFind.maxLevel
        ? maxLevel
        : hotmObjectToFind.maxLevel;
    let powderSum,
    ...
    else powderSum = findCost(hotmName, minLevel, maxLevel);

    ChatLib.chat("");
    ChatLib.chat(
      `&6${
        hotmObjectToFind.nameStringed
      } ${minLevel} - ${maxLevel} &bwill cost &6&l${addCommas(
        Math.round(powderSum)
      )} &6${
        hotmObjectToFind.powderType[0].toUpperCase() +
        hotmObjectToFind.powderType.slice(1)
      } &bpowder.`
    );
    ChatLib.chat(
      `&6${
        hotmObjectToFind.nameStringed
      } ${minLevel} - ${maxLevel} &bwill give &6&l${addCommas(
        Math.round(reward * 100) / 100
      )} &bof whatever reward is listed.`
    );
    ChatLib.chat("");
  }).start();
}
...
export function findHotmObject(hotmName) {
  ...
  for (let i = 0; i < hotmData.length; i++) {
    if (hotmData[i].names.includes(hotmName.toLowerCase())) return hotmData[i];
  }
}

let tickData = JSON.parse(
  FileLib.read("BetterColeweight", "data/tickData.json")
);
...
export function findCost(hotmName, minLevel, maxLevel, fortunate = false) {
  ...
  if (tickData[`${hotmName} ${minLevel}-${maxLevel}`] != undefined)
    ...
    if (!fortunate) costFormula = findHotmObject(hotmName).costFormula;

    let powderSum = 0;

    if (fortunate) powderSum = Math.pow(maxLevel + 1, 3.05);
    else {
      for (
        let currentLevel = minLevel;
        currentLevel <= maxLevel;
        currentLevel++ // finds cost
      )
