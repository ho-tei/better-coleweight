...
export function calcSpeed(powder) {
  ...
  if (powder == undefined || parseNotatedInput(powder) == undefined)
  ...
  while (powder > msPowder(speedLevels) + profPowder(professionalLevels)) {
    if (
      ms2SpeedPerPowder(speedLevels + 1) >
        professionalSpeedPerPowder(professionalLevels + 1) &&
      speedLevels < 50
    ) {
  ...
  return ChatLib.chat(
    `&bGet &6&l${speedLevels} &bmining speed levels and &6&l${professionalLevels} &bprofessional levels.`
  );
}
