...
/**
 * ! May be removed in the future, I don't see a proper use case for this
 */

registerCommand({
  aliases: ["optimize"],
  description:
    "Optimizes routes for Skytils (next neighbour algorithm, y values are 2.5x the distance). Routes must only contain vein nodes! no bombs! This is probably only good for coal/long routes like coal because of the different variables when doing other forms of mining. &4&lDeprecated",
  options: "(skytils route category name)",
  category: "miscellaneous",
  execute: (argsT) => {
    // thanks rAnalysis for helping.
    args = argsT;
    mainThread.start();
  },
});
...
const mainThread = new Thread(() => {
  ...
  let routes = JSON.parse(
    FileLib.read("BetterColeweight", "../../../skytils/waypoints.json")
  );
  let route = routes.categories.find(
    (route) =>
      route.name.toLowerCase() === args.slice(1).join(" ").toLowerCase()
  );
  let origWaypoints = route?.waypoints;
  if (origWaypoints == undefined || origWaypoints.length < 2)
    return ChatLib.chat(
      `${PREFIX}&cRoute does not exist or there is less than two waypoints.`
    );

  let nextNeighborObj = nextNeighbor(origWaypoints);
  let newWaypoints = nextNeighborObj.waypoints,
    distances = nextNeighborObj.distances;
  ...
  for (let i = 0; i < origWaypoints.length; i++)
    console.log(
      origWaypoints[i].x + "," + origWaypoints[i].y + "," + origWaypoints[i].z
    );
  for (let i = 0; i < newWaypoints.length; i++)
    console.log(
      newWaypoints[i].x + "," + newWaypoints[i].y + "," + newWaypoints[i].z
    );

  ChatLib.command(`ct copy ${Base64.encode(JSON.stringify(output))}`, true);

  ChatLib.chat(
    `${PREFIX}&bRoute copied to clipboard! Original distance: ` +
      Math.round(distances.orig) +
      " Optimized distance: " +
      Math.round(distances.new)
  );
});
...
function nextNeighbor(origWaypoints) {
  ...
  for (let i = 1; i < origWaypoints.length; i++) {
    ...
    else distances.orig += distanceCalc(origWaypoints[i], origWaypoints[i + 1]);
    ...
    for (let i = 0; i < newWaypoints.length - 2; i++) {
      for (let j = i + 2; j < newWaypoints.length - 1; j++) {
        ...
        if (d1 + d2 > d3 + d4) {
          ...
          let newTour = newWaypoints
            .slice(0, i + 1)
            .concat(newWaypoints.slice(i + 1, j + 1).reverse())
            .concat(newWaypoints.slice(j + 1));
