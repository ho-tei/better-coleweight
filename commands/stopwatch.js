...
export default registerCommand({
  ...
  execute: (args) => {
    if (args[1] == undefined)
      return ChatLib.chat(
        `${constants.PREFIX}&bOptions are: &3set&b, &3reset&b, &3show&b, &3pause&b.` // TODO: Add reference to options list
      );
    if (args[1].toLowerCase() == "set") {
      if (args[2] == undefined) constants.data.stopwatchGui.stopwatch = 0;
      else if (args[2].includes("h"))
        constants.data.stopwatchGui.stopwatch =
          parseInt(args[2]) * 60 * 60 ?? 0;
      else if (args[2].includes("m"))
        constants.data.stopwatchGui.stopwatch = parseInt(args[2]) * 60 ?? 0;
      else constants.data.stopwatchGui.stopwatch = parseInt(args[2]) ?? 0;

      constants.data.tickStopwatch = true;
      ChatLib.chat(
        `${constants.PREFIX}&bSet stopwatch to ${Math.floor(
          constants.data.stopwatchGui.stopwatch / 60
        )}m ${Math.ceil(constants.data.stopwatchGui.stopwatch)}s`
      );
      ...
      ChatLib.chat(
        `${constants.PREFIX}&b${Math.floor(
          constants.data.stopwatchGui.stopwatch / 60
        )}m ${Math.ceil(constants.data.stopwatchGui.stopwatch)}s`
      );
    else if (
      args[1].toLowerCase() == "pause" ||
      args[1].toLowerCase() == "unpause"
    ) {
