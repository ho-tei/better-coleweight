...
registerCommand({
  ...
  execute: (args) => {
    ...
    if (username == undefined) {
      ChatLib.chat(`${PREFIX}&eRequires a username!`);
      return;
    }
    request({
      ...
      .then((mojangRes) => {
        request({
          ...
          .then((hypixelRes) => {
            let discordMessage = new TextComponent(
              `${PREFIX}&a${mojangRes.name}'s Discord: `
            );
          ...
          .catch((err) => {
            ChatLib.chat(
              `${PREFIX}&eNo discord linked :( (or no key linked)${
                settings.debug ? " " + err : ""
              }`
            );
          });
