  ...
  execute: (args) => {
    switch ((args[1] ?? "").toLowerCase()) {
      ...
      case "help":
        ...
        helpCommand(
          "calculate tick",
          "Calculates tick data.",
          "(mining speed) (('r','jade', etc) || breaking power of block))"
        );
        helpCommand(
          "calculate speed",
          "Calculates the ratio of mining speed 2 to professional with a certain amount of powder.",
          "(powder)"
        );
        helpCommand(
          "calculate hotm",
          "Calculates powder between two levels of a certain perk.",
          "(perk) (minlevel) [maxlevel]"
        );
        ChatLib.chat("&b------------------------------------------");
        return;
      default:
