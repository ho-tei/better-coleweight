...
export function registerCommand(command) {
  ...
  if (command.showInHelp ?? true)
    helpCommands[command.category].push({
...
register("command", (...args) => {
  ...
  commands.forEach((command) => {
    if (
      (command.cw ?? true) &&
      command.aliases.includes(args[0].toString().toLowerCase())
    ) {
  ...
  if (!stop)
    ChatLib.chat(
      `${constants.PREFIX}&bUnknown command. Type "/bcw help" to see all commands.`
    );
})
  .setTabCompletions((args) => {
    ...
    commands.forEach((command) => {
      if (
        ...
        for (
          let i = 0;
          i < command.subcommands.length && i <= args.length - 1;
          i++
        )
