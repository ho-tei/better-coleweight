...
function checkMMiners() {
  if (
    !settings.trackGriefers ||
    (!settings.grieferEverywhere &&
  ...
  try {
    ...
    PlayerMap.filter(
      (player) =>
        player.func_178853_c() > 0 &&
        !player.func_178845_a().name.startsWith("!")
    ).forEach((PlayerMP) => {
      ...
      if (!checkedPlayers.includes(player)) {
        ...
        if (griefer.found) {
          ChatLib.chat(
            `${PREFIX}&e'${player}' has griefed before. Their last grief was on ${griefer.dateObj
              .toString()
              .slice(4, 15)}.`
          );
    ...
    if (settings.debug) console.log("grieferTrack trycatch: " + err);
  }
...
register("gameLoad", () => {
  axios
    .get("https://ninjune.dev/api/mminers")
    .then((res) => {
