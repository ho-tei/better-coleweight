...
register(
  ...
  (
    ...
    if (
      !settings.rankEverywhere &&
      !(dwarvenChecker.getState() || hollowsChecker.getState())
    )
      return;
    ...
    playerMessage.split(" ").forEach((chunk) => {
      if (chunk.startsWith("https")) onward = false;
      else if (chunk.startsWith("[ITEM:")) onward = false;
    });
    ...
    (message = ChatLib.getChatMessage(event, true)), cancel(event);
    ...
    messagePrefix =
      ...
      ` &8[§${playerRank}#${addCommas(cwlbPlayerData.rank)}&8]&f: `;

    newMessage.addTextComponent(messagePrefix);

    if (hypixelRank == "" && typeOfChat == "")
      playerMessage = "&7" + playerMessage.slice(0);
    else playerMessage = "&f" + playerMessage.slice(0);

    newMessage.addTextComponent(playerMessage);
    ChatLib.chat(newMessage);
  }
).setCriteria(
  /^(\[\d+\] )?((?:(?:Guild|Party|Co-op) > )|(?:\[:v:\] ))?(.)? ?(\[\w+\+{0,2}\] )?(\w{1,16}) ?(♲)?(?: \[\w{1,6}\])?: (.*)$/g
);

register("gameLoad", () => {
  axios
    .get("https://ninjune.dev/api/coleweight-leaderboard?length=500")
    .then((res) => {
      cwlbData = res.data;
    })
    .catch((err) => {
...
export function reloadColor() {
  axios.get("https://ninjune.dev/api/cw/colors/get-players").then((res) => {
...
register("step", () => {
  if (constants.data.first_time) {
    ...
    ChatLib.chat("");
    new TextComponent(
      ChatLib.getCenteredText(`${PREFIX}&bView commands: /bcw help`)
    ).chat();
    new TextComponent(
      ChatLib.getCenteredText(
        `${PREFIX}&bJoin my discord &3here&b to keep up with development!`
      )
    )
