...
registerWhen(
  register("renderPlayerList", (event) => {
    if (timers.playerList <= 0) {
      ...
      ChatLib.chat(
        `${constants.PREFIX}&bBCW has canceled opening tab! (disable &aStreamer Mode&b in settings if you don't want this.)`
      );
    }
...
registerWhen(
  register("renderDebug", (event) => {
    if (timers.debug <= 0) {
      ChatLib.chat(
        `${constants.PREFIX}&bBCW has canceled opening debug menu! (disable &aStreamer Mode&b in settings if you don't want this.) (Hit F3 to stop this message spam)`
      );
      timers.debug = 20;
    }
    cancel(event);
...
register("chat", (event) => {
  if (!(settings.streamerMode && settings.streamerDisableWaypointsOnDeath))
    return;
  ChatLib.chat(
    `${constants.PREFIX}&bBCW has disabled your waypoints because you died! "/bcw ordered enable" to turn them back on. (disable &aStreamer mode&b to disable this feature)`
  );
  ChatLib.command("bcw ordered disable", true);
}).setChatCriteria(/^[&r&c]* . [&r&7]*You.*/g);

// ? Doesn't this interfere with the marking lobby?
