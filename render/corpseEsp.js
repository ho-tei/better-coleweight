...
registerWhen(
  register("renderWorld", () => {
    const entities = World.getAllEntitiesOfType(EntityArmorStand.class).filter(
      (a) => a?.getName() == "Armor Stand" && !a.isInvisible()
    );

    for (let i = 0; i < entities.length; i++) {
      let helmetName = new EntityLivingBase(entities[i].getEntity())
        .getItemInSlot(4)
        ?.getName()
        ?.removeFormatting();
      if (
        claimed.some((e) => entities[i].getPos().distance(e) < 5) ||
        !helmetName
      )
        continue;
      let text, rgb;
      ...
      drawCoolWaypoint(
        Math.floor(entities[i].getRenderX()),
        Math.floor(entities[i].getRenderY()),
        Math.floor(entities[i].getRenderZ()),
        rgb[0],
        rgb[1],
        rgb[2],
        { name: text, showDist: true }
      );
    }
  }),
  () => {
