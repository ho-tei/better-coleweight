...
registerWhen(
  register("postGuiRender", (mX, mY, gui) => {
    if (
      !gui.toString().includes("GuiChest") ||
      !Player.getContainer().getName().toLowerCase().includes("recipe")
    )
      return;
    const container = Player.getContainer();
    const supercraft = container.getStackInSlot(32);
    if (!supercraft?.getName()?.includes("Supercraft")) return;
    const lore = supercraft.getLore();
    let ret = false;
    lore.forEach((l) => {
      if (l.includes("Max")) ret = true;
    });
    if (ret) return;
    lore.splice(0, 1);
    const regres = lore[0]
