...
const collectionGui = new BaseGui(
  ...
  () => {
    ...
    if (
      !(
        settings.showCollectionTrackerAlways ||
        trackingItem ||
        trackedItem == ""
      )
    )
    ...
    if (settings.collectionNotation)
      rightValues = [
        addNotation("oneLetters", currentItem) ?? 0,
        addNotation("oneLetters", itemPerHour) ?? 0,
        addNotation("oneLetters", itemValuesSum) ?? 0,
        secondsToMessage(uptimeSeconds),
      ];
    else
      rightValues = [
        addCommas(currentItem) ?? 0,
        addCommas(itemPerHour) ?? 0,
        addCommas(itemValuesSum) ?? 0,
        secondsToMessage(uptimeSeconds),
      ];
  ...
  () => {
    return settings.collectionTracker || collectionGui.isOpen();
  },
  resetVars
);
...
register("step", () => {
  ...
  if (trackingItem == true) uptimeSeconds += 1;
  if (seconds == 0 || seconds == 15 || seconds == 30 || seconds == 45)
    calcApi(
      ["members", Player.getUUID().replace(/-/g, ""), "collection"],
      Player.getUUID()
    );
}).setFps(1);
    ...
    request({
      ...
      .then((res) => {
        for (let i = 0; i < res.profiles.length; i += 1) {
          if (res.profiles[i].selected == true) profileData = res.profiles[i];
        }
        let source = getObjectValue(profileData, apiPath)[trackedItem];

        if (currentItem == 0 || currentItem == undefined) {
          currentItem = source;
        } else if (trackingItem && source - currentItem > 0) {
          // don't track first item because it won't have time tracked.
...
function trackCollection(collection) {
  let collections = JSON.parse(
    FileLib.read("BetterColeweight", "data/collections.json")
  );
  if (collection == undefined)
    return ChatLib.chat(
      `${constants.PREFIX}&eThat is not a valid collection! (or is not supported)`
    );
  if (collection == "obby") collection = "obsidian";
  if (collection == "cobble") collection = "cobblestone";
  if (collections[collection.toLowerCase()] == undefined)
    return ChatLib.chat(
      `${constants.PREFIX}&eThat is not a valid collection! (or is not supported)`
    );
  constants.data.tracked.item = collections[collection].collectionToTrack;
  constants.data.tracked.itemStringed =
    collections[collection].collectionStringed;
  constants.data.save();

  ChatLib.chat(
    `${constants.PREFIX}&bSet collection to ${constants.data.tracked.itemStringed}!`
  );
}
