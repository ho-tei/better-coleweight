...
/**
 * ? Why is this in a mining mod?
 */


const danceGui = new BaseGui(
  ["danceGui", "dance"],
  () => {
    ...
    blocks.forEach((block) => {
      if (
        World.getBlockAt(block[0], block[1], block[2]).type.getRegistryName() ==
        "minecraft:stained_glass"
      )
        dance = true;
    });

    if (danceIndex < 1) return;
    ...
    if (
      2000 - (modifiedTime - danceTime) < 1400 &&
      2000 - (modifiedTime - danceTime) > 200
    )
      punches = 0;
    ...
    const currentTime =
      Math.floor((2000 - (modifiedTime - danceTime)) / 1000 / 0.05) * 0.05;
    const nextTime = currentTime + 2;
    let message = `${danceData[danceIndex - 1]}&7: &b${Math.max(
      currentTime,
      0
    ).toFixed(2)}\n${danceData[danceIndex]}&7: &b${nextTime.toFixed(2)}`;
    return `${danceIndex} / ${danceData.length - 1}${
      danceStreak > 0 ? " Streak: " + danceStreak : ""
    }`;
  },
  () => {
...
register("renderTitle", (title, subtitle, event) => {
  if (!settings.danceTracker || !checkInMirrorverse()) return;
  if (subtitle.includes("§bMove!§r") && danceIndex == 0) {
    ...
    danceTime = Date.now();
    cancel(event);
  } else if (danceIndex >= 1) cancel(event);
});
...
register("renderWorld", () => {
  ...
  let currentTime =
    Math.floor((2000 - (Date.now() - danceTime)) / 1000 / 0.05) * 0.05;

  if (danceIndex == 1) currentTime -= 1;
...
register("renderOverlay", () => {
  ...
  if (
    ...
    2000 - (Date.now() - danceTime) <= 1500
  ) {
    const txt = `${Player.isSneaking() ? "&aSneaking!" : "&cSneak!"}`;
    let sneakText = new Text(
      txt,
  ...
  if (
    ...
    2000 - (Date.now() - danceTime) <= 500
  ) {
    const txt = `${
      !Player.getPlayer().field_70122_E ? "&aJumping!" : "&cJump!"
    }`;
    let jumpText = new Text(
      txt,
      Renderer.screen.getWidth() / 2,
  ...
  if (currentTime >= punchMin && currentTime < punchMax) return;
  const txt = `${punches != 0 ? "&aPunched!" : "&cPunch!"}`;
  let punchText = new Text(
    txt,
    Renderer.screen.getWidth() / 2,
...
register("chat", (message) => {
  if (
    !(
      (message.startsWith("You wer") || message.startsWith("You didn")) &&
      message.endsWith("!")
    )
  )
    return;
...
register("soundPlay", (pos, name, volume, pitch, category, event) => {
  if (pitch != 0.6984127163887024 || name != "note.bassattack") return;
