...
import {
  ...
  endChecker,
  hollowsChecker,
  isPlayerHoldingDrill,
  Title,
} from "../../util/helperFunctions";
import { BaseGui } from "../BaseGui";

const miningAbilitiesGui = new BaseGui(
  ["abilityGui", "miningabilities"],
  () => {
    ...
    activeAbilities.forEach((ability) => {
      if (
        settings.miningAbilitiesSelectedIndicator &&
        ability.name == selectedAbility
      )
        leftValues.push(`&e${ability.name} CD`);
      else leftValues.push(`${ability.name} CD`);
...
function checkAreas() {
  if (dwarvenChecker.check() || hollowsChecker.check() || endChecker.check())
    return true;
  return false;
}
...
register("step", () => {
  activeAbilities.forEach((ability) => {
    if (ability.timer > 0) ability.timer -= 1;
...
register("chat", (abilityName, event) => {
  ...
  /&r&aYou selected &r&e(.+) &r&aas your Pickaxe Ability. This ability will apply to all of your pickaxes!&r/g
);
...
function addAbility(abilityName, timer = 0) {
  ...
  if (!found) {
    let object = {
      timer,
      name: capitalizeFirst(abilityName),
      title: new Title({
        text: `&6[&3&kd&6] &b&l${capitalizeFirst(abilityName)}&6 [&3&kd&6]`,
      }),
      maxTimer,
    };
