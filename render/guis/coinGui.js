...
const coinGui = new BaseGui(
  ["coinGui", "coin", "money", "cointracker"],
  () => {
    ...
    return `&a(${lastGemstone} ${type} @ &b${addCommas(
      lastPrice
  ...
  () => {
    return coinGui.isOpen() || settings.coinTracker;
  },
  resetVars
);
registerGui(coinGui);
...
register("chat", (gem, amount, event) => {
        ...
        Object.keys(res.products)
          .filter((i) => {
            if (
              i.startsWith("FLAWED") ||
              i.startsWith("FINE") ||
              i.startsWith("FLAWLESS") | i.startsWith("PERFECT") ||
              i.startsWith("ROUGH")
            )
              return true;
          })
          .forEach((i) => {
            ...
            gemstoneCosts[i] = settings.forceNPC
              ? npc
              : Math.max(npc, res.products[i].quick_status.sellPrice);
          });
      })
      .catch((err) => {
        if (settings.debug) console.log("Coin tracker: " + err);
      });
    return;
  }
  ...
  money +=
    (gemstoneCosts[id] / Math.pow(80, 3 - settings.gemstoneType)) * amount;
  moneyPerHour = Math.floor(
    money / ((Date.now() - startTime) / (1000 * 60 * 60))
  );
}).setChatCriteria(
  /&r&d&lPRISTINE! &r&fYou found &r&a. Flawed (.+) Gemstone &r&8x(\d+)&r&f!&r/g
);
