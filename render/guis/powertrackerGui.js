...
const powderGui = new BaseGui(
  ["powdertrackerGui", "powdertracker", "powder"],
  () => {
    ...
    if (settings.showTotals) {
      addText(
        "Total Chest",
        addCommas(constants.data.powdertrackerGui.chests),
        lines
      );
      addText(
        "Total Gemstone",
        addNotation(
          "oneLetters",
          constants.data.powdertrackerGui.gemstonePowder
        ),
        lines
      );
      addText(
        ...
        addNotation(
          "oneLetters",
          constants.data.powdertrackerGui.mithrilPowder
        ),
        lines
      );
    }
    ...
    addText(
      "Session Gemstone",
      addNotation("oneLetters", sessionGemstone),
      lines
    );
    addText(
      "Session Mithril",
      addNotation("oneLetters", sessionMithril),
      lines
    );
    ...
    if (settings.showRates) {
      addText(
        "Chests/hr",
        addCommas(Math.round(((sessionChests ?? 0) / (seconds ?? 1)) * 3600)),
        lines
      );
      addText(
        "Gemstone/hr",
        addNotation(
          "oneLetters",
          Math.round(((sessionGemstone ?? 0) / (seconds ?? 1)) * 3600)
        ),
        lines
      );
      addText(
        ...
        addNotation(
          "oneLetters",
          Math.round(((sessionMithril ?? 0) / (seconds ?? 1)) * 3600)
        ),
        lines
      );
      if (uptimeHr >= 1)
        addText(
          "Uptime",
          `${uptimeHr}h ${Math.floor(seconds / 60) - uptimeHr * 60}m`,
          lines
        );
      else
        addText(
  ...
  () => {
    return powderGui.isOpen() || settings.trackerVisible;
  },
  resetVars
);
...
function DoublePowderActive() {
  const bossBar = Java.type(
    "net.minecraft.entity.boss.BossStatus"
  )?.field_82827_c;
  if (bossBar == undefined) return false;
...
register("chat", (value, type) => {
  ...
  if (DoublePowderActive()) powder *= 2;
