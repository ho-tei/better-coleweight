...
registerWhen(
  register("step", () => {
    if (countSludge()) sludgesInvisible = true;
  }).setDelay(1),
  () => {
...
registerWhen(
  register("renderEntity", (entity, position, partialTicks, event) => {
    ...
    if (entity instanceof EntitySlime) cancel(event);
    else if (
...
function countSludge() {
  if (!hollowsChecker.check()) return false;
  let count = 0;

  World.getAllEntitiesOfType(EntityArmorStand).forEach((stand) => {
    if (stand.getName().includes("Sludge")) count++;
