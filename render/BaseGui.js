  ...
  constructor(aliases, renderOverlayCallback, dependency, reloadCallback) {
    ...
    registerWhen(
      register("renderOverlay", () => {
        if (this.gui.isOpen()) {
          let txt =
            "Drag to move. Use +/- to increase/decrease gui size. Use arrow keys to set alignment.";
        ...
        if (this.gui.isOpen())
          message =
            `&f&os: ${constants.data[aliases[0]].scale.toFixed(2)}&r\n` +
            (message ?? "");
        let text = new Text(message ?? "");
        text.setX(constants.data[aliases[0]].x);
        text.setY(
          constants.data[aliases[0]].y -
            (this.gui.isOpen() ? 10 * constants.data[aliases[0]].scale : 0)
        ); // addition to account for "s: ..."
        if (constants.data[aliases[0]].alignment == 1) text.setAlign("CENTER");
    ...
    register("guiKey", (char, keyCode, gui, event) => {
      ...
      if (keyCode == 13) constants.data[aliases[0]].scale += 0.05;
      else if (keyCode == 12) constants.data[aliases[0]].scale -= 0.05;
      else if (keyCode == 203) constants.data[aliases[0]].alignment = 0;
      else if (keyCode == 208 || keyCode == 200)
        constants.data[aliases[0]].alignment = 1;
      else if (keyCode == 205) constants.data[aliases[0]].alignment = 2;
