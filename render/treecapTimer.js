...
registerWhen(
  register("renderOverlay", () => {
    ...
    if (
      !(
        itemId == '"TREECAPITATOR_AXE"' ||
        itemId == '"ASPECT_OF_THE_VOID"' ||
        Player?.getHeldItem()?.getRegistryName() == "minecraft:fishing_rod"
      )
    )
      return;
    ...
    Renderer.drawStringWithShadow(
      ...
      Renderer.screen.getHeight() / 2 - Renderer.screen.getHeight() / 25
    );
  }),
  () => {
...
registerWhen(
  register("step", () => {
    if (treecapCooldown > 0) {
      ...
      if (treecapCooldown - 0.1 * multipler < 0) treecapCooldown = 0;
      else treecapCooldown -= 0.1 * multipler;
    }
  }).setFps(10),
  () => {
    return settings.treecapTimer && foragingChecker.check();
...
register("blockBreak", (block) => {
  ...
  if (
    treecapCooldown > 0.5 ||
    !(
      blockRegistryName == "minecraft:log" ||
      blockRegistryName == "minecraft:log2"
    )
  )
    return;
  treecapCooldown = 2.0;
...
register("chat", (lvl, pet, event) => {
  ...
  /&cAutopet &eequipped your &.\[Lvl ([0-9]+)] &.([a-zA-Z ]+)(?: .)*&e! &a&lVIEW RULE&r/g
);
